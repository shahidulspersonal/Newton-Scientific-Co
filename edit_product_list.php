...
<div class="container mx-auto mt-6">
   <h1 class="text-center text-3xl font-bold mb-6">Product List</h1> <!-- Removed text-gray-50 class -->

   <!-- Search Form -->
   <form class="flex justify-center mb-6 text-black" action="" method="get">
      <input class="form-input mr-2 p-2 border border-gray-300 rounded" type="text" name="search" placeholder="Search Product" aria-label="Search">
...
<div class="container mx-auto px-4">
   ...
   while($row = mysqli_fetch_assoc($select)) {
      ...
      echo '<td class="py-2 px-4 text-white">'.$row['name'].'</td>'; // 
      echo '<td class="py-2 px-4 text-white">'.$row['origin'].'</td>'; //
      echo '<td class="py-2 px-4 text-white">'.$row['details'].'</td>'; 
      echo '<td class="py-2 px-4">';
      echo '<a href="edit_product.php?id='.$row['id'].'" class="bg-yellow-500 hover:bg-yellow-700 text-white py-1 px-3 rounded">Edit</a>';
      echo '</td>';
      echo '</tr>';
   }
